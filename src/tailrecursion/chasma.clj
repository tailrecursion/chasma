...
(defn spawn
  ...
  (^Actor [] (spawn (current-universe!) (constantly nil)))
  (^Actor [u-or-beh]
   (if (instance? Universe u-or-beh)
     (spawn u-or-beh (constantly nil))
     (spawn (current-universe!) u-or-beh)))
  (^Actor [u beh]
   (let [u (ensure-universe u)] (->Actor u (UUID/randomUUID) (atom beh)))))
...
(defrecord Delivery [target ^Actor to from payload ^Serializer serializer
                     retries])
...
(defn- target-delivery
  [target]
  (cond (instance? Lane target) [(:universe target) target (:target target)
                                 (:serializer target)]
        (instance? Actor target) [(:universe target) target target nil]
        :else (throw (IllegalArgumentException.
                       "Expected an actor or lane target"))))

(defn- put-delivery!
  [^Delivery d]
  (let [^LinkedBlockingQueue queue (:queue (delivery-universe d))]
    (.put queue d)))
  ...
  (let [pairs (vec (cons [act new] (partition 2 more)))
    ...
    (doseq [[actor _] pairs]
      (when-not (instance? Actor actor)
        (throw (IllegalArgumentException.
                 "become! requires actor/behavior pairs")))
...
(defn- lock-actors!
  ...
  (letfn [(step [actors]
            (if-let [actor (first actors)]
              (locking actor (step (next actors)))
              (f)))]
    (step (sort-by :id actors))))

(defn- run-commit-effects!
  [^Universe u effects]
  (doseq [f effects]
    (try (binding [*tx* nil] (f))
         (catch Throwable t ((:effect-error-handler (:opts u)) t)))))

(defn- validate-staged-sends!
  [sends]
  (doseq [^Delivery d sends] (ensure-running! (delivery-universe d))))
    ...
    (lock-actors! actors
                  (fn []
                    ...
                    (doseq [{:keys [^Actor actor new]} becomes]
                      (reset! (.-beh actor) new))))
    (doseq [^Delivery d sends] (put-delivery! d))
    (run-commit-effects! (:universe tx) @(:effects tx))))

(defn- apply-become-pairs!
  [u pairs]
  ...
  (if *tx*
    ...
    (loop []
      (let [tx (new-tx u)]
        (enqueue-becomes! tx pairs)
        (when (try (commit! tx) false (catch Throwable _ true)) (recur))))))
...
(defn ask
  ...
  (let [[u target actor serializer] (target-delivery target)
        ...
        me (spawn u)]
    (reset! (.-beh me) (fn [v] (.complete fut v) (become! me (fn [& _] nil))))
...
(defmacro on-commit!
  ...
  [& body]
  `(if *tx*
     (do (swap! (:effects *tx*) conj (binding [*tx* nil] (bound-fn [] ~@body)))
         nil)
...
(defn- backoff-ms
  ...
  (let [{:keys [retry-base-ms retry-max-ms]} (:opts u)]
    (min retry-max-ms
         (long (* retry-base-ms (Math/pow 2.0 (max 0 (dec (double n)))))))))
...
(defn- run-attempt
  [^Delivery d]
  (try (run-turn! d) (catch Throwable _ (retry-delivery d))))

(defn- run-with-retries!
  [^Delivery d]
  (loop [d d]
    (when @(:running? (delivery-universe d))
      (when-let [retry (run-attempt d)] (recur retry)))))

(defn- next-serialized-delivery
  [^Serializer s]
  (locking s
    (if-let [d (.pollFirst ^ArrayDeque (:queue s))]
      d
      (do (reset! (:running? s) false) nil))))

(defn- drain-serializer!
  [^Universe u ^Serializer s]
  (loop []
    (when @(:running? u)
...
(defn- schedule-serializer!
  [^Universe u ^Serializer s]
  (when-let [^ExecutorService executor @(:pool u)]
    (.submit executor ^Runnable #(drain-serializer! u s))))

(defn- enqueue-serialized!
  [^Universe u ^Serializer s ^Delivery d]
  (ensure-running! u)
  (let [start? (locking s
                 (.addLast ^ArrayDeque (:queue s) d)
                 (compare-and-set! (:running? s) false true))]
    (when start? (schedule-serializer! u s))))

(defn- run-delivery!
  [^Delivery d]
  (when-let [retry (run-attempt d)] (enqueue! retry)))

(defn- pump-loop
  [^Universe u]
  (future (let [^LinkedBlockingQueue queue (:queue u)
...
(defn stop!
  ...
  (let [u (ensure-universe u)]
    (when (compare-and-set! (:running? u) true false)
      ...
      (when-let [^ExecutorService executor @(:pool u)] (.shutdownNow executor))
      (.clear ^LinkedBlockingQueue (:queue u))
      (reset! (:pump u) nil)
      (reset! (:pool u) nil))
    u))
