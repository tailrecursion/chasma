...
(defn- struct-instance-of?
  [expected-def arg]
  (and (instance? PersistentStructMap arg)
       (identical? expected-def (struct-def arg))))
...
(defn- applicable?
  ...
  (case (:kind spec)
    ...
    :struct (struct-instance-of? (:struct spec) arg)
...
(defn ^:no-doc make-generic
  ...
  (let [generic (atom {:name name,
    ...
    (with-meta (fn [& args] (invoke-generic generic args))
      {:chasma/generic true, :generic generic, :name name})))
